...
class CompositeTrapezoidal extends Component {
    ...
    CompositeTrapezoidal(a, b, n) {
        var h = (b-a)/n
        I = (h / 2) * (this.func(a) + this.func(b) + 2*this.Func_sum(n, h+a))
    ...
    dataapi = async()=>{
        var response = await axios.get('http://localhost:3001/api/users/showtrapcomposite').then(res => {return res.data});
        this.setState({
