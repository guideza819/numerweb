...
class Central_h4 extends Component {
    constructor() {
        super();
        this.state = {
    ...
    Central_h4(x, h, degree) {
        if(degree===1){
            ans = (-this.func(x+(2*h)) + 8*this.func(x+(1*h)) - 8*this.func(x-(1*h)) + this.func(x-(2*h))) / (12*h)
        }
        if(degree===2){
            ans = (-this.func(x+(2*h)) + 16*this.func(x+(1*h)) - 30*this.func(x) + 16*this.func(x-(1*h)) - this.func(x-(2*h))) / (12*Math.pow(h, 2))
        }
        if(degree===3){
            ans = (-this.func(x+(3*h)) + 8*this.func(x+(2*h)) - 13*this.func(x+(1*h)) + 13*this.func(x-(1*h)) - 8*this.func(x-(2*h)) + this.func(x-(3*h))) / (8*Math.pow(h, 3))
        }
        if(degree===4){
            ans = (-this.func(x+(3*h)) + 12*this.func(x+(2*h)) - 39*this.func(x+(1*h)) + 56*this.func(x) - 39*this.func(x-(1*h)) + 12*this.func(x-(2*h)) + this.func(x-(3*h))) / (6*Math.pow(h, 4))
        }
    ...
    dataapi = async()=>{
        ...
        this.Central_h4(this.state.x,this.state.h,this.state.degree);
    }
    render() {
        return(
            <body style={{background: "#bae7ff", padding: "90px" , float:"left" }}>
                <h2 style={{color: "#003a8c", fontWeight: "bold",fontSize: "35px"}}>Central Divided-Differences O(h<sup>4</sup>)</h2>
                <div style={{float:"left"}}>
                    ...
                    >
                        ...
                        <Button id="submit_button" onClick= {
                                ()=>this.Central_h4(parseFloat(this.state.x), parseFloat(this.state.h), parseInt(this.state.degree))
                            }  
