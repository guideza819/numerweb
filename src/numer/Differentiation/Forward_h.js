...
class Forward_h extends Component {
    constructor() {
        super();
        this.state = {
    ...
    Forward_h(x, h, degree) {
        if(degree===1){
            ans = (this.func(x+(1*h)) - this.func(x)) / h
        }
        if(degree===2){
            ans = (this.func(x+(2*h)) - 2*this.func(x+(1*h)) + this.func(x)) / Math.pow(h, 2)
        }
        if(degree===3){
            ans = (this.func(x+(3*h)) - 3*this.func(x+(2*h)) + 3*this.func(x+(1*h)) - this.func(x)) / Math.pow(h, 3)
        }
        if(degree===4){
            ans = (this.func(x+(4*h)) - 4*this.func(x+(3*h)) + 6*this.func(x+(2*h)) - 4*this.func(x+(1*h)) + this.func(x)) / Math.pow(h, 4) 
        }
    ...
    dataapi = async()=>{
        ...
        this.Forward_h(this.state.x,this.state.h,this.state.degree);
    }
    render() {
        return(
            <body style={{background: "#bae7ff", padding: "90px" , float:"left" }}>
                <h2 style={{color: "#003a8c", fontWeight: "bold",fontSize: "35px"}}>Forward Divided-Differences O(h)</h2>
                <div style={{float:"left"}}>
                    ...
                    >
                        ...
                        <Button id="submit_button" onClick= {
                                ()=>this.Forward_h(parseFloat(this.state.x), parseFloat(this.state.h), parseInt(this.state.degree))
                            }  
